...
<head>
    ...
    <script>
        ...
        function kotChanged(type){
            document.getElementsByName('token_item_id')[0].style.display = type !== '' ? 'inline-block' : 'none'
        }
    </script>
</head>
<body>
<form method="post" >@csrf
    ...
    <select name="kot_type" onchange="kotChanged(this.value)" style="display:@if(request()->input('model') !== 'Kitchen') none @endif"><option value="" @if(request()->input('kot_type') === '') selected @endif></option><option value="_add" @if(request()->input('kot_type') === '_add') selected @endif>Add</option><option value="_modify" @if(request()->input('kot_type') === '_modify') selected @endif>Modify</option><option value="_cancel" @if(request()->input('kot_type') === '_cancel') selected @endif>Cancel</option></select>
    <input type="text" name="token_item_id" placeholder="token item id" value="{{ request()->input('token_item_id') }}" style="display:@if(request()->input('model') !== 'Kitchen' || trim(request()->input('kot_type')) === '') none @endif">
...
@php
    ...
    $data = (method_exists($model,$method)) ? call_user_func_array([$model,$method],[$model,request()->input('token'),request()->input('token_item_id')]) : (method_exists($model,'print_data') ? call_user_func_array([$model,'print_data'],[$model,request()->input('token'),request()->input('token_item_id')]) : $model);
