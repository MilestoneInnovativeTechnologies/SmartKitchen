...
<form method="post" >@csrf
    ...
    <input type="text" name="template_name" placeholder="Template Name" value="{{ request()->input('template_name','kot_print_template') }}">
    <input type="text" name="token" placeholder="If Kitchen, then type here token id" value="{{ request()->input('token',1) }}">
...
@php
    ...
    $data = (method_exists($model,$method)) ? call_user_func_array([$model,$method],[$model,request()->input('token')]) : (method_exists($model,'print_data') ? call_user_func_array([$model,'print_data'],[$model,request()->input('token')]) : $model);
