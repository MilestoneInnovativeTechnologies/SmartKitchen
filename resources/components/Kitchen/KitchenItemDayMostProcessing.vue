...
export default {
  ...
  computed: {
    ...
    items(){ let today = _.map(this.today,'id') || []; return _.map(_.groupBy(_.flatMap(this.tokens,({ items,id }) => _.includes(today,id) ? _.filter(items,({ kitchen,progress }) => kitchen && _.includes(this.kitchens[_USER.id],kitchen.id) && progress !== 'Cancelled') : []),'item.id'),(items,item) => _.zipObject(
      ['item','quantity','time'],
