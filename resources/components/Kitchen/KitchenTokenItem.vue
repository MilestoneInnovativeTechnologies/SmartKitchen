...
<!--    <q-card-section class="text-center text-bold">{{ total ? ('Total: ' + total) : 'No Items' }}</q-card-section>-->
    <q-card-section class="row items-center q-py-xs">
      <div class="text-primary">Stock</div><q-space />
      <q-avatar size="sm" text-color="white" rounded color="primary">{{ iStock }}</q-avatar><q-space />
      <div class="text-primary">Total</div><q-space />
      <q-avatar size="sm" text-color="white" rounded color="primary">{{ total }}</q-avatar>
    </q-card-section>
...
export default {
  ...
  methods: {
    ...
    itmObj(itmAry){ let itm = itmAry[0]; return Object.assign({},this.pure(itm),{ total: _.sumBy(itmAry,'quantity'),token: _(itmAry).keyBy('token').mapValues(({ token,id,user,quantity,type }) => new Object({ token,id,user,quantity,type })).value() }) },
