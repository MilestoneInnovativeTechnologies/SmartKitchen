  ...
  <q-card class="q-pa-none">
    ...
    <q-list v-if="items.length" separator>
      <q-item v-for="(item,idx) in items" :key="hKey(item,idx)">
        <q-item-section avatar><q-badge :label="item.token" color="primary" class="q-pa-sm shadow-2" /></q-item-section>
        <q-item-section>
          <q-item-label><span class="text-weight-bolder">{{ item.quantity }}</span> x {{ item.item_name }}</q-item-label>
          <q-item-label caption :class="kColor(item)">{{ item.kitchen_name }}</q-item-label>
...
export default {
  ...
  computed: {
    assign(){ return this.$store.getters['kitchens/assign'][parseInt(_USER.id)] },
    ...mapState({
      kItems: function({ kitchens:{ items } }){ return _(_.pick(items,this.assign)).flatMap().groupBy('item').mapValues(this.iGroup).value() },
  ...
  methods: {
    iGroup(iAry){ return _(iAry).map(({ item,kitchen,stock }) => _.zipObject(['item','kitchen','stock','item_name','kitchen_name'],[item,kitchen,stock,_.get(this.IK,['item',item]),_.get(this.IK,['kitchen',kitchen])])).value() },
