  ...
  <q-card>
    ...
    <template v-if="reference">
      <q-card-section class="q-pb-none row items-center q-col-gutter-x-xs">
        <span :class="'text-' + color" class="text-h6 text-bold">{{ offline_reference }}</span><q-space />
...
export default {
  ...
  computed: {
    offline_reference(){ return _.get(_.find(this.$store.state.remote.data,{ item:'token_items',local_id:parseInt(this.id) }),['extra','offline_reference']) },
