  ...
  <q-page padding>
    <q-list separator>
      <q-item v-for="(token,idx) in Tokens" :key="h_key('ao-o-',token.id)" clickable @click="show = !!((selected = idx) || true)">
        <q-item-section avatar><q-avatar rounded size="md" text-color="white" color="green">{{ token.id }}</q-avatar></q-item-section>
...
export default {
  ...
  computed: {
    Tokens(){ return _(this.tokens_own).filter(({ type }) => this.display.includes(type)).sortBy(({ date }) => extract_date(date).getTime()).reverse().value() },
