...
export default {
  ...
  computed: {
    ...
    table(){ return _(this.bills).filter(({ date,progress }) => progress !== 'Cancelled' && is_between(date,this.range.from,this.range.to)).map(({ date,payable,paid,user }) => _.zipObject(['Time','User','Payable','Paid','Balance'],[
      time(date),_.get(user,'name'),precision(payable,2),precision(paid,2),precision(payable-paid,2)
    ])).value() },
