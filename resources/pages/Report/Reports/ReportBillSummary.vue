...
export default {
  ...
  computed: {
    ...
    table(){ return _(this.bills).filter(({ date,progress }) => progress !== 'Cancelled' && is_between(date,this.range.from,this.range.to)).map(({ date,customer:{ name },payable,paid }) => _.zipObject(['Time','Customer','Payable','Paid','Balance'],[
      time(date),name,precision(payable,2),precision(paid,2),precision(payable-paid,2)
    ])).value() },
