...
export default {
  ...
  computed: {
    ...
    remote_order_items(){ return _(this.bills).filter(({ progress,date,token }) => token.type === 'Remote' && progress !== 'Cancelled' && is_between(date,this.range.from,this.range.to)).flatMap('token.items').filter(({ item }) => item.id === this.item.id).value() },
