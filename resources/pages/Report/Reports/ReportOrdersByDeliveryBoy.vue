...
export default {
  name: "PageReportOrdersByDeliveryBoy",
  components: {ReportCard},
  mixins: [Bills],
  computed: {
    range(){ return range(this.$store.state.public.range) },
    head(){ return _.zipObject(['Report','Start Date','End Date'],['Tokens By Delivery Boy',to_format('ddd DD/MMM hh:mm',this.range.from),to_format('ddd DD/MMM hh:mm',this.range.to)]) },
    ...
    user_ids(){ return _.map(this.$store.getters['users/role_users']('Delivery Boy'),'id') },
