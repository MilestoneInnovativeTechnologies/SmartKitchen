...
export default {
  name: "PageReportOrdersByReceptionist",
  components: {ReportCard},
  mixins: [Bills],
  computed: {
    range(){ return range(this.$store.state.public.range) },
    head(){ return _.zipObject(['Report','Start Date','End Date'],['Tokens By Receptionist',to_format('ddd DD/MMM hh:mm',this.range.from),to_format('ddd DD/MMM hh:mm',this.range.to)]) },
    ...
    user_ids(){ return _.map(this.$store.getters['users/role_users']('Receptionist'),'id') },
    user_tokens(){ return _(this.tokens).filter(({ user }) => _.includes(this.user_ids,user)).filter(({ date,progress,waiter }) => progress !== 'Cancelled' && waiter && is_between(date,this.range.from,this.range.to)).groupBy('waiter.name').mapValues(tokens => _.map(tokens,({id}) => _.get(this.token_amounts,id,{ amount:0,payable:0 }))).value() },
    table(){
      return _.map(this.user_tokens,(tokens,Waiter) => Object(
