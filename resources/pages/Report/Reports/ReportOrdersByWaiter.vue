...
export default {
  ...
  computed: {
    ...
    waiter_ids(){ return _.map(this.$store.getters['users/role_users']('Waiter'),'id') },
    waiter_tokens(){ return _(this.tokens).filter(({ user }) => _.includes(this.waiter_ids,user)).filter(({ date,progress,waiter }) => progress !== 'Cancelled' && waiter && is_between(date,this.range.from,this.range.to)).groupBy('waiter.name').mapValues(tokens => _.map(tokens,({id}) => _.get(this.token_amounts,id,{ amount:0,payable:0 }))).value() },
    table(){
