...
export { arrayRemove,onSnapshot,setDoc,getDoc } from "firebase/firestore";
...
export function remote(item,prop){
  ...
  return new Promise(async function (resolve,reject){
    ...
    if(typeof prop === 'string') return doc(FDB,reference + '/' + item + '/data/' + prop)
  })
}
...
export function remote_query(item,qry){ return new Promise((resolve,reject) => remote_ref(item).then(ref => getDocs(query(ref,...(_.map(proper_query(qry),args => where(...args))))).then(resolve).catch(reject)).catch(reject)) }
